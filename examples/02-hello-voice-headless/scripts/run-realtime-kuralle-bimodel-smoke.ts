...
//
// Live bi-model gate: gpt-realtime-2 front delegates to kuralle agent (fromKuralleRuntime)
// via ask_university; measures delegate latency + first-audio V2V.
...
const OUTPUT_DIR = join(PKG_ROOT, "test", "performance", "runs", "realtime-kuralle-bimodel-smoke");
...
interface BimodelMetrics {
  readonly delegateLatencyMs: number | null;
  readonly firstAudioV2vMs: number | null;
  readonly firstAudioLeadInV2vMs: number | null;
  readonly frontLeadIn: {
    readonly beforeToolResult: boolean;
    readonly firstLeadInAtMs: number | null;
    readonly toolResultAtMs: number | null;
    readonly leadInKind: "tts.audio" | "assistant_transcript" | "none";
  };
}
...
function computeMetrics(
  ...
  lastUserAudioAtMs: number | null,
): BimodelMetrics {
  const toolCall = timeline.find((e) => e.event === "bus.llm.tool_call");
  const toolResult = timeline.find((e) => e.event === "bus.llm.tool_result");
  const firstTts = timeline.find((e) => e.event === "bus.tts.audio");
  const firstAssistant = timeline.find((e) => e.event === "adapter.assistant_transcript.final");

  const toolCallAtMs = toolCall?.atMs ?? null;
  const toolResultAtMs = toolResult?.atMs ?? null;
  const firstTtsAtMs = firstTts?.atMs ?? null;
  const firstAssistantAtMs = firstAssistant?.atMs ?? null;

  const delegateLatencyMs =
    toolCallAtMs !== null && toolResultAtMs !== null ? toolResultAtMs - toolCallAtMs : null;

  const firstAudioV2vMs =
    lastUserAudioAtMs !== null && firstTtsAtMs !== null ? firstTtsAtMs - lastUserAudioAtMs : null;
  const firstAudioLeadInV2vMs =
    lastSpeechFrameAtMs !== null && firstTtsAtMs !== null ? firstTtsAtMs - lastSpeechFrameAtMs : null;

  let leadInKind: BimodelMetrics["frontLeadIn"]["leadInKind"] = "none";
  let firstLeadInAtMs: number | null = null;

  if (firstTtsAtMs !== null && (firstAssistantAtMs === null || firstTtsAtMs <= firstAssistantAtMs)) {
    leadInKind = "tts.audio";
    firstLeadInAtMs = firstTtsAtMs;
  } else if (firstAssistantAtMs !== null) {
    leadInKind = "assistant_transcript";
    firstLeadInAtMs = firstAssistantAtMs;
  }

  const beforeToolResult =
    toolResultAtMs !== null &&
    firstLeadInAtMs !== null &&
    firstLeadInAtMs < toolResultAtMs;

  return {
    delegateLatencyMs,
    firstAudioV2vMs,
    firstAudioLeadInV2vMs,
    frontLeadIn: {
      beforeToolResult,
      firstLeadInAtMs,
      toolResultAtMs,
...
async function main(): Promise<void> {
  ...
  let lastSpeechFrameAtMs: number | null = null;
  let lastUserAudioAtMs: number | null = null;
  ...
  const completion = new Promise<void>((resolve, reject) => {
    ...
    const hardTimeout = setTimeout(() => {
      reject(new Error("realtime kuralle bimodel smoke timeout"));
    }, 180_000);
  ...
  while (offset < pcm.length) {
    ...
    lastUserAudioAtMs = Date.now() - startedAt;
    offset += FRAME_SAMPLES;
    await sleep(20);
  }
  lastSpeechFrameAtMs = lastUserAudioAtMs;
  pushTimeline("user.audio.last_speech_frame", String(lastSpeechFrameAtMs));
  ...
  pushTimeline("user.audio.last_frame", String(lastUserAudioAtMs));
  ...
  const metrics = computeMetrics(timeline, lastSpeechFrameAtMs, lastUserAudioAtMs);
  ...
  console.log(`\n=== BI-MODEL PASS: ${pass ? "YES" : "NO"} ===`);
  console.log(`delegate latency (tool_result - tool_call): ${metrics.delegateLatencyMs}ms`);
  console.log(`first-audio V2V (first tts.audio - last injected frame): ${metrics.firstAudioV2vMs}ms`);
  console.log(`first-audio lead-in V2V (first tts.audio - last speech frame): ${metrics.firstAudioLeadInV2vMs}ms`);
  console.log(
    `front lead-in before tool_result: ${metrics.frontLeadIn.beforeToolResult} ` +
      `(kind=${metrics.frontLeadIn.leadInKind}, leadInAt=${metrics.frontLeadIn.firstLeadInAtMs}ms, ` +
      `toolResultAt=${metrics.frontLeadIn.toolResultAtMs}ms)`,
  );
