...
//
// Live CF bi-model gate: connect to deployed syrinx-voice-realtime-workers,
// stream university CS masters deadline audio, capture grounded response.
...
const OUTPUT_DIR = join(PKG_ROOT, "test", "performance", "runs", "realtime-cf-bimodel-smoke");
...
function deployedBaseUrl(): string {
  const fromEnv = process.env["SYRINX_CF_REALTIME_URL"]?.trim();
  if (fromEnv) return fromEnv.replace(/\/$/, "");
  throw new Error("SYRINX_CF_REALTIME_URL is required (e.g. https://syrinx-voice-realtime-workers.<account>.workers.dev)");
}
...
async function runSmoke(deployedUrl: string): Promise<SmokeResult> {
  const sessionId = `cf-bimodel-${randomUUID()}`;
...
async function main(): Promise<void> {
  ...
  console.log(`\n=== CF BI-MODEL PASS: ${result.ok ? "YES" : "NO"} ===`);
  ...
  if (!result.ok) {
    throw new Error("CF bi-model smoke failed — missing grounded March 31 or silent audio");
  }
}
