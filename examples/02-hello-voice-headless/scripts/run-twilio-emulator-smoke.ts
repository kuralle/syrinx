...
const TWILIO_SAMPLE_RATE_HZ = 8000;
const STREAM_SID = "MZ-emulated-stream";
const CALL_SID = "CA-emulated-call";
const CONTEXT_ID = `twilio-${CALL_SID}`;
...
const EXPECTED_OUTBOUND_BYTES = EXPECTED_OUTBOUND_FRAMES * TWILIO_SAMPLE_RATE_HZ * OUTBOUND_FRAME_DURATION_MS / 1000;
...
async function main(): Promise<void> {
  ...
  const runDir = join(RUNS_DIR, `twilio-emulator-${runId}`);
  ...
  let inboundSendStats: InboundSendStats = {
    ...
    maxMediaGapMs: 0,
    networkProfile: readNetworkProfile(),
  };
  let inboundBytes = 0;
  ...
  const socket = new WebSocket(`ws://127.0.0.1:${address.port}/twilio`);
  ...
  try {
    ...
    socket.send(JSON.stringify({
      ...
      start: {
        ...
        mediaFormat: {
          ...
          sampleRate: TWILIO_SAMPLE_RATE_HZ,
          channels: 1,
        },
      },
    }));
    ...
    if (firstPayloadBytes !== 160) {
      throw new Error(`Expected one 20 ms PCMU frame, got ${firstPayloadBytes} bytes`);
    }
    ...
    socket.send(JSON.stringify({ event: "stop", streamSid: STREAM_SID, stop: { accountSid: "AC-emulated" } }));
    ...
    if (failures.length > 0) throw new Error(`twilio emulator smoke failed: ${failures.join("; ")}`);
  } finally {
...
function evaluateResult(result: SmokeResult): string[] {
  ...
  if (result.inboundFrames !== EXPECTED_INBOUND_FRAMES) {
    failures.push(`expected ${EXPECTED_INBOUND_FRAMES} inbound Twilio media frames, got ${result.inboundFrames}`);
  }
  ...
  if (result.outboundFrames !== EXPECTED_OUTBOUND_FRAMES) {
    failures.push(`expected ${EXPECTED_OUTBOUND_FRAMES} outbound Twilio media frames, got ${result.outboundFrames}`);
  }
  if (result.outboundBytes !== EXPECTED_OUTBOUND_BYTES) {
    failures.push(`expected ${EXPECTED_OUTBOUND_BYTES} outbound Twilio media bytes, got ${result.outboundBytes}`);
  }
  if (result.outboundMarks !== 2) failures.push(`expected two outbound Twilio marks, got ${result.outboundMarks}`);
  if (result.outboundEndMarks !== 1) failures.push(`expected one outbound Twilio terminal mark, got ${result.outboundEndMarks}`);
...
function buildSmokeManifest(args: {
  ...
  return {
    ...
    audio: {
      inputSampleRateHz: INPUT_SAMPLE_RATE_HZ,
      outputSampleRateHz: TWILIO_SAMPLE_RATE_HZ,
      ...
      outputDurationMs: pcMuDurationMs(args.result.outboundBytes, TWILIO_SAMPLE_RATE_HZ),
    },
    turns: [
      {
        id: CONTEXT_ID,
        ...
        assistantAudio: {
          ...
          durationMs: pcMuDurationMs(args.result.outboundBytes, TWILIO_SAMPLE_RATE_HZ),
        },
        latencyMs: {
...
async function sendEmulatedPhoneAudio(socket: WebSocket, startedAt: number): Promise<InboundSendStats> {
  ...
  const interFrameDelaysMs = interFrameDelays(networkProfile);
  const frameSamples = TWILIO_SAMPLE_RATE_HZ * 20 / 1000;
  const audio = generateTone(TWILIO_SAMPLE_RATE_HZ, 440, INPUT_DURATION_SECONDS);
  let wireBytes = 0;
  ...
  return { wireBytes, firstMediaAfterStartMs, lastMediaAfterStartMs, maxMediaGapMs, networkProfile };
}
...
async function waitForJson(
  ...
  return await new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      ...
      reject(new Error("Timed out waiting for Twilio websocket message"));
    }, timeoutMs);
