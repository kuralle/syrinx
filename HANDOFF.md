...
- `@asyncdot/voice` uses TTS packet sample-rate metadata when estimating assistant playback duration for idle timeout scheduling. Legacy plugins without metadata still default to 24 kHz, but maintained Cartesia and Gemini plugins now emit their actual provider PCM rates.
