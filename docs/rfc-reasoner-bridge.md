...
>
> **v2.3 changelog (Sprint 3 — suspend/resume architecture, spike-verified):** §4.6 rewritten. Cross-turn `resumeStream(runId)` requires **Mastra's own durable snapshot store** (not a hand-rolled run-state row) — proven by `examples/spikes/mastra-edge` (FEASIBLE-WITH-CAVEATS) against `@mastra/core@1.41.0` + `@mastra/cloudflare@1.4.1`. Design: a **dedicated Mastra Cloudflare Worker DO** (`nodejs_compat`, wrangler-bundled ~7.8 MB → **Workers Paid**) with `CloudflareDOStorage({ sql: ctx.storage.sql })`; our `RunStore` shrinks to a `{contextId → runId}` pointer; the main AI-SDK worker stays Mastra-free + edge-clean. §9 edge-weight risk quantified.
>
...
> **v2.3 amendment (Sprint 3 — verified by the `examples/spikes/mastra-edge` spike against `@mastra/core@1.41.0` + `@mastra/cloudflare@1.4.1`).** The original "the only new state is one SQL row keyed by `runId`" was wrong: **Mastra owns the workflow snapshot** — `agent.resumeStream(data,{runId})` reloads it from the *Mastra instance's configured storage* (`AGENT_RESUME_NO_SNAPSHOT_FOUND` if the store is in-memory/volatile across processes). So cross-turn resume requires Mastra to persist its own snapshot durably. The verified design:
>
> - **Mastra runs on a dedicated Cloudflare Worker DO** (not the lean AI-SDK product worker), configured with **`@mastra/cloudflare` `CloudflareDOStorage({ sql: ctx.storage.sql })`** — Mastra persists its workflow snapshot in the DO's SQLite, so a suspended run resumes on a fresh Mastra instance over the same `ctx.storage.sql` after hibernation (spike Q3: PASS). This worker uses **`nodejs_compat`** (Mastra imports `events`/`fs`/`path`/`crypto`; bundled via **wrangler**, ~7.8 MB / 1.3 MB gz → **Workers Paid tier**). The main AI-SDK worker stays Mastra-free + `verify-edge-bundle.sh`-clean; the strict edge gate applies to **it**, not the Mastra worker.
> - **Our `RunStore` is a pointer, not the run state:** a `{contextId → pending runId}` row (the only thing the *voice-turn routing* needs — "this conversation has a pending run"). Mastra's `CloudflareDOStorage` holds the actual checkpoint. Discard the pointer on resume-complete or barge-in.
> - **fs caveat:** `@mastra/core` bundles `fs`/`@ast-grep/napi` (workspace tools) — not hit on the suspend/resume path, but the edge Mastra agent must **not** use workspace/file tools (they'd fail on Workers' no-filesystem).

A `suspended` part is terminal: the bridge speaks `prompt` (if any), records the `{contextId → runId}` pointer via the injected `RunStore`, ends the turn, and emits a `reasoning.suspended` packet. On the next user turn the orchestrator recognizes a pending run for the conversation and feeds a `ReasonerTurn` with `resume: { runId, data: mappedUserAnswer }`; the Mastra adapter calls `agent.resumeStream(data, {runId})`, which reloads the snapshot from `CloudflareDOStorage`. The DO + its SQLite survive hibernation between turns, so the run resumes after eviction. A barged-in suspended run is discarded (drop the pointer; the Mastra snapshot is GC'd by TTL). This reuses the existing turn loop + DO; the new state is the Mastra snapshot (Mastra-owned, in `ctx.storage.sql`) plus our one pointer row.
...
- **Edge-bundle weight (Mastra) — quantified (v2.3):** `@mastra/core` + `@mastra/cloudflare` bundle to **~7.8 MB (1.3 MB gz)** via wrangler with `nodejs_compat` (raw esbuild `--platform=browser` fails — Mastra imports `events`/`fs`/`path`/`crypto`). Resolution: **Sprint 2** keeps Mastra off the lean AI-SDK product worker (Node path; `verify-edge-bundle.sh` green). **Sprint 3** puts Mastra on a **dedicated** Cloudflare Worker DO (`nodejs_compat`, Workers **Paid** tier — 7.8 MB exceeds the 3 MiB Free limit, within the 10 MiB Paid limit) for the suspend/resume path; that worker has its own relaxed bundle build, the product worker stays clean. A bundle diet (narrow `@mastra/core` entry, exclude workspace/deployment tooling) is a backlog optimization to reach Free tier. `fs`/`@ast-grep/napi` are bundled but must not be exercised on edge (no Workers filesystem).
