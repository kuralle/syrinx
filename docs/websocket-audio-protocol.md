...
The supplied browser review console sends microphone audio as `syrinx.audio.v1` binary envelopes by default, not JSON base64. The browser client library attaches monotonic sequence metadata to JSON/base64, PCM envelope, and Float32 envelope audio send paths by default, and rejects duplicate/regressing explicit sequence overrides before send. JSON audio frames remain supported for scripted clients when they carry explicit source-rate metadata.
