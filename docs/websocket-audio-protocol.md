...
The server validates strict base64, requires PCM16 payloads to have an even byte length, and resamples from `sampleRateHz` to `inputSampleRateHz` before pushing `user.audio_received` into the engine.
