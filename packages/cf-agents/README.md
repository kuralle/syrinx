...
| `onTurn` | `(ctx: { sessionId, turnId, userText?, assistantText?, consulted, ts }) => void \| Promise<void>` — fired once per **realtime** conversational turn, whether or not it consulted the Reasoner. `consulted` distinguishes a grounded turn (a delegate call occurred) from one the front model answered alone, so a host writing one analytics row per turn does not need to correlate `onDelegateResult` by `turnId` itself. Not fired on cascade pipelines. Throwing never affects the call. |
