...
| `onTurn` | `(ctx: { sessionId, turnId, userText?, assistantText?, consulted, ts }) => void \| Promise<void>` — fired once per **realtime** conversational turn, whether or not it consulted the Reasoner. `consulted` is true when the front *issued* a delegate call in this turn — recorded at the call, not the result, so a NON_BLOCKING tool call whose answer arrives after the turn commits still reports `true` — distinguishing a grounded turn from one the front model answered alone, so a host writing one analytics row per turn does not need to correlate `onDelegateResult` by `turnId` itself. Not fired on cascade pipelines. Throwing never affects the call. |
