...
| C3 | Backchannels — RESHAPED per the codex decision (`research/interaction-policy/c3-backchannel-decision.md`): a conservative asset-backed **wait-gap cue layer** composing with the shipped v4.1.0 thinking bed, NOT a broad backchannel system. Cue-id (not free text); rule-based emits ≤1 cue on G3 `tool_call_cue.delayed` (no user-pause backchannels); pre-cached PCM cues via BackgroundAudioMixer; gated by `caps.emitsBackchannel` + audio/user-speech/asset checks; VAP owns continuous timing later. NO free-form tool-call filler fields. | `interaction-policy.ts` (cue-id + `toolCallPhase` obs), `interaction-coordinator.ts`, `packets.ts` (`InteractionBackchannelPacket`), `policies/rule-based.ts` (gap state machine), `packages/server-websocket/src/background-audio.ts` (one-shot cue) + outbound paths, `packages/cf-agents/src/with-voice.ts` | **planned** — brief-C3.md ready; fires to grok after C4/C5. Placeholder cues + listen-smoke; real voice-matched cues = follow-up |
