...
It stages live provider secrets from `.env` into the bot app, deploys both apps with `--ha=false`, runs Twilio, Telnyx, and SmartPBX shaped calls by default, downloads evidence locally, then destroys both Fly apps in a `finally` block. After download it validates the local evidence bundle: internally consistent provider quality gates, provider-specific completion evidence, bot recorder `manifest.json` using the `@asyncdot/voice-recorder` manifest contract, `events.jsonl`, user/assistant PCM byte counts, bot WAV headers/sample rates/data lengths, carrier call result JSON matching the summary `callResult`, and carrier inbound/outbound WAV headers plus decoded PCM data lengths at 8 kHz. This is the run to use when real carrier accounts are unavailable; the provider-account sections above validate account setup, not the core engine path. The summary is written under:
